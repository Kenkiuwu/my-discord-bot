...
raid_groupings = {}
TZ = timezone(timedelta(hours=2))  # UTC+2 aka ST
...
async def group_generation_task():
    ...
    if now.weekday() == 1 and now.hour == 20 and 0 <= now.minute < 5:
        # Generate groups for Brelshaza Hardmode with the fixed roster
        await generate_groups("brel_hm")

        # Generate groups for other raids (FIFO)
        for raid in ["brel_n", "aegir_n", "aegir_h"]:
            await generate_groups(raid)

        # Notify players about their groups
...
async def admin_view_groups(interaction: discord.Interaction):
    ...
    for raid, groups in raid_groupings.items():
        ...
        for i, group in enumerate(groups, start=1):
            message += f"🔹 Group {i}:\n"
            for p in group:
                message += f"- {p['user']} | {p['character']} ({p['day']} at {p['start_time']} ST)\n"
    await interaction.response.send_message(message, ephemeral=True)

@tree.command(name="homework_availability", description="Set your general availability for homework raids")
@app_commands.describe(entries="Multiple entries as 'Day Start End, Day Start End', e.g., 'Wed 18:00 21:00, Fri 20:00 23:00'")
async def homework_availability_cmd(interaction: discord.Interaction, entries: str):
    ...
    try:
        ...
        await interaction.response.send_message("❌ Invalid format. Use: 'Wed 18:00 21:00, Fri 20:00 23:00'", ephemeral=True)
        return
...
@app_commands.describe(raid="Raid name (brel_n, aegir_h, aegir_n)", characters="Characters and ilvl, e.g. 'Souleater 1670 Artillerist 1680'")
...
async def generate_groups(raid):
    ...
    used = set()
    if raid == "brel_hm":
        # Fixed 8-man Brelshaza Hardmode Group
        group = []
        for user in FIXED_ROSTER:
            ...
            group.append({
                "user": user,
                "display_name": display_name,
                "character": "Fixed Role",  # Set fixed character names for the roster
                "ilvl": "TBD",  # Set ilvl or leave it for future adjustments
                "day": "Tuesday",  # Set to match your fixed timing
                "start_time": "20:00"  # Set fixed time for the raid
            })
        groups.append(group)
    else:
        # FIFO for other raids
        for s in supports:
            if s["user"] in used:
                continue
            for s2 in supports:
...
bot.run("MTM2ODIzMzk2Mzk3NzcwMzU2NQ.GdEJqp.w-Ud5wpx45_LEQW-JWKKdlBrqkPkxn0UeTa6Ng")